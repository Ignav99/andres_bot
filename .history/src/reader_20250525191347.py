...
def parse_calendar(path_excel: str) -> List[Dict[str, Any]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        # 1) Encontrar fila de Mes-Año (primer bloque en filas 1–10)
        month_row = None
        for r in range(1, 11):
            for cell in ws[r]:
        ...
        for col in range(1, max_col + 1):
            ...
            if isinstance(cell.value, str) and " - " in cell.value:
                ...
                if len(parts) == 2:
                    nombre_mes, anyo = parts[0], int(parts[1])
                    current_my = (nombre_mes, anyo)
            if current_my:
                month_info[col] = current_my

        # 3) Detectar fila de días: buscamos la primera con varios números distintos 1–31
        day_row = None
        for r in range(month_row + 1, month_row + 8):
            valores = [c.value for c in ws[r] if isinstance(c.value, (int, float))]
            if len(set(valores)) > 1 and all(1 <= int(v) <= 31 for v in valores):
                day_row = r
                break
        if not day_row:
            continue
