...
def parse_calendar(path_excel: str, target_date: Optional[date]=None) -> List[Dict[str, Any]]:
    ...
    for sheet in wb.sheetnames:
        ...
        for r in range(1, 11):
            for c in ws[r]:
                if isinstance(c.value, str) and re.search(r"^[A-Z]+ ?-? ?\d{4}$", c.value.strip(), re.IGNORECASE):
        ...
        for r in range(6, ws.max_row+1):
            ...
            for col in cols:
                ...
                # B) Color de fondo (incluso condicional)
                else:
                    raw = None
                    if fill:
                        # primero intentamos start_color.rgb
                        raw = getattr(fill.start_color, "rgb", None)
                        # si no, probamos fgColor.rgb
                        if not raw:
