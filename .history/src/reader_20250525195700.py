...
def parse_calendar(path_excel: str) -> List[Dict[str, Any]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        # 3) Detectar fila de días: la primera (a partir de row 4) con al menos 4 números distintos entre 1 y 31
        day_row = None
        for r in range(4, ws.max_row + 1):
            nums = [c.value for c in ws[r] if isinstance(c.value, (int, float))]
            uniq = set(nums)
            if len(uniq) >= 4 and all(1 <= int(v) <= 31 for v in uniq):
                day_row = r
                break
        if not day_row:
            # no encontramos la fila de días válida → saltar hoja
            continue
