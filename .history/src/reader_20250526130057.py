...
def parse_calendar(
    ...
    for sheet in wb.sheetnames:
        ...
        # 1) Detectar fila de mes-año: siempre filas 1–6
        month_row = None
        for r in range(1, 7):
            for c in ws[r]:
                val = c.value
                # Reconoce formatos tipo "Month - 2025" o "MONTH - 2025"
                if isinstance(val, str) and "-" in val and re.search(r"\b\d{4}\b", val):
                    month_row = r
                    break
            if month_row:
                # Si es Repsol y acabamos de encontrar en fila 2, priorizamos:
                if empresa.lower() == "repsol" and month_row == 2:
                    break
                # Si no es Repsol y hemos encontrado en cualquier fila, salimos
                if empresa.lower() != "repsol":
                    break
                # Si es Repsol y encontrámos en otra fila que no sea la 2,
                # seguimos buscando hasta la 2, pero no más allá de la 6.
                if empresa.lower() == "repsol" and r < 2:
                    # seguimos para ver si la 2 es válida
                    continue
                else:
                    break
            else:
