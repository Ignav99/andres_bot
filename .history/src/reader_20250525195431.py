...
def parse_calendar(path_excel: str) -> List[Dict[str, Any]]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        # 3) Detectar fila de días: la primera que tenga al menos 3 valores distintos 1–31
        day_row = None
        for r in range(month_row + 1, month_row + 10):
            nums = [c.value for c in ws[r] if isinstance(c.value, (int, float))]
            uniq = set(nums)
            # requerimos al menos 3 días distintos (evitamos la fila de todos "1")
            if len(uniq) >= 3 and all(1 <= int(v) <= 31 for v in uniq):
                day_row = r
                break
        if not day_row:
            # no encontramos la fila de días → saltar hoja
            continue
