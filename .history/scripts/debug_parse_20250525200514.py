...
def debug_sheet(name):
    ...
    # 3) Fijamos la fila de días en la 5 (donde efectivamente aparecen 1,2,3,…)
    day_row = 5
    # Validación opcional: asegurarnos de que hay varios días distintos
    vals = [c.value for c in ws[day_row] if isinstance(c.value, (int, float))]
    if not (len(set(vals)) >= 3 and all(1 <= int(v) <= 31 for v in vals)):
        # Si por alguna razón no es la fila correcta, saltamos esta hoja
        continue
